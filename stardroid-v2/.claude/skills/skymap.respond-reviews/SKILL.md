...
Package name: `com.google.android.stardroid` — this is a **single Play Store listing shared by
both `stardroid-v1/` and `stardroid-v2/`** (v2 is designed to ship as an update
to the same listing). Reviews may describe behavior from whichever
build is currently live in production — don't assume from this skill's location that reviews
are about v2 specifically. This is a copy of v1's `skymap.respond-reviews` skill kept in sync in
both module trees since the review stream and troubleshooting docs are shared, not per-module.
...
complaints or issues — not needed for purely positive reviews. Both files live at the repo root
(`stardroid/`), one level up from either `stardroid-v1/` or `stardroid-v2/` —
they describe user-facing symptoms and are not tied to either codebase's internals.
