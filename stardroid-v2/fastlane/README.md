...
Run unit tests, ktlint, and the Konsist architecture gate via `check`
...
### android sync_version_code_from_play_store

```sh
[bundle exec] fastlane android sync_version_code_from_play_store
```

Sync local version code to the Play Console before incrementing, in case another upload (a different checkout, a manual upload) advanced it past our gradle file
