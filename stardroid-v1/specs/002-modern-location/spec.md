...
- **FR-001**: The app MUST request only coarse (network-quality) location permission; fine/precise location MUST NOT be requested.
...
- Coarse location accuracy (city-level) is sufficient for a star map; fine-grained GPS precision is explicitly not required.
