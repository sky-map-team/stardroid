...
- [x] T011 [P] [US1] Create `app/src/main/java/com/google/android/stardroid/activities/ImageGalleryActivityModule.kt`: Hilt `@Module` providing activity-scoped dependencies, following the pattern in `DynamicStarMapActivityModule`
- [x] T012 [P] [US1] Ensure `ImageGalleryActivity.kt` is annotated with `@AndroidEntryPoint` and its module is installed in `ActivityComponent`.
- [x] T013 [US1] Create `app/src/main/java/com/google/android/stardroid/activities/ImageGalleryActivity.kt`: `@AndroidEntryPoint` annotated Activity that injects `ObjectInfoRegistry`; in `onCreate` calls `registry.getAllWithImages()`, sets `RecyclerView` with `GridLayoutManager(this, 3)` and `GalleryAdapter`; replace the TODO in `DynamicStarMapActivity.java` (T003) with a real launch Intent
